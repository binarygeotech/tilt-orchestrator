...
import { Trash2, Settings, Package, Database as DatabaseIcon, GitBranch } from 'lucide-react';
...
export default function ServiceCard({ service, onToggle, onDelete, onEdit }: ServiceCardProps) {
  ...
  return (
    <Card className={`transition-opacity ${!service.enabled ? 'opacity-60' : ''}`}>
      ...
      <CardContent className="pt-0 space-y-2">
        ...
        {service.env && Object.keys(service.env).length > 0 && (
          <div className="text-sm text-slate-600 dark:text-slate-400">
            <span className="font-medium">Env vars:</span> {Object.keys(service.env).length}
          </div>
        )}
        {service.docker && (
          <div className="text-sm text-slate-600 dark:text-slate-400">
        ...
        {service.k8s && (
          <div className="text-sm text-slate-600 dark:text-slate-400">
            <span className="font-medium">Manifests:</span> {service.k8s.manifests}
          </div>
        )}
        {service.kustomize && (
          <div className="text-sm text-slate-600 dark:text-slate-400">
            <span className="font-medium">Kustomize:</span> {service.kustomize.path}
          </div>
        )}
      </CardContent>
    </Card>
