...
import { Plus, Trash2, GitBranch, Download, Loader2 } from 'lucide-react';
...
export default function AddServiceDialog({ 
  ...
  const [deploymentTypes, setDeploymentTypes] = useState<Array<'docker' | 'k8s' | 'helm' | 'kustomize'>>([]);
  ...
  useEffect(() => {
    if (initialService) {
      ...
      if (initialService.env) {
        setEnvVars(Object.entries(initialService.env).map(([key, value]) => ({ key, value })));
      }
  ...
  const handleSubmit = async () => {
    ...
    // Reset form
    setService({ name: '', port: 8080, enabled: true, env: {}, repo: { url: '', branch: 'main' } });
    setEnvVars([]);
    setDependencies([]);
    setDeploymentTypes([]);
    setShouldClone(true);
    ...
    <Dialog open={open} onOpenChange={(newOpen) => {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing ? 'Update the service configuration' : 'Configure a new service for this environment'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="8080"
                value={service.port}
                onChange={(e) => setService({ ...service, port: parseInt(e.target.value) || 8080 })}
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            {cloneError && (
              <p className="text-sm text-red-600 dark:text-red-400">{cloneError}</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="has-repo" className="cursor-pointer flex items-center gap-2">
          ...
          {/* Repository Info */}
          {hasRepo && (<div className="space-y-4 p-4 border rounded-lg bg-slate-50 dark:bg-slate-900">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setService({
                      ...service,
                      repo: { url: e.target.value, branch: service.repo?.branch || 'main' },
                    })
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setService({
                      ...service,
                      repo: { url: service.repo?.url || '', branch: e.target.value },
                    })
                  }
                />
              </div>
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="should-clone" className="cursor-pointer flex items-center gap-2">
                ...
                <Checkbox
                  ...
                  onCheckedChange={(checked) => {
                    if (checked) {
                      ...
                      if (!service.docker) {
                        setService({ ...service, docker: { context: '.', dockerfile: 'Dockerfile' } });
                      }
                    } else {
                      setDeploymentTypes(deploymentTypes.filter(t => t !== 'docker'));
                    }
                  }}
                />
                <Label htmlFor="docker-type" className="cursor-pointer">Docker</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="k8s-type"
                  ...
                  onCheckedChange={(checked) => {
                    if (checked) {
                      ...
                      setDeploymentTypes(deploymentTypes.filter(t => t !== 'k8s'));
                    }
                  }}
                />
                <Label htmlFor="k8s-type" className="cursor-pointer">Kubernetes</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="helm-type"
                  ...
                  onCheckedChange={(checked) => {
                    if (checked) {
                      ...
                      if (!service.helm) {
                        setService({ ...service, helm: { chart: '', namespace: 'default' } });
                      }
                    } else {
                      setDeploymentTypes(deploymentTypes.filter(t => t !== 'helm'));
                    }
                  }}
                />
                <Label htmlFor="helm-type" className="cursor-pointer">Helm</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="kustomize-type"
                  ...
                  onCheckedChange={(checked) => {
                    if (checked) {
                      ...
                      if (!service.kustomize) {
                        setService({ ...service, kustomize: { path: 'kustomize' } });
                      }
                    } else {
                      setDeploymentTypes(deploymentTypes.filter(t => t !== 'kustomize'));
                    }
                  }}
                />
                <Label htmlFor="kustomize-type" className="cursor-pointer">Kustomize</Label>
              </div>
            </div>
          </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setService({
                        ...service,
                        docker: { context: e.target.value, dockerfile: service.docker?.dockerfile || '' },
                      })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setService({
                        ...service,
                        docker: { dockerfile: e.target.value, context: service.docker?.context || '' },
                      })
                    }
                  />
                </div>
            ...
            <div className="space-y-4 p-4 border rounded-lg bg-green-50 dark:bg-green-950">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setService({ ...service, k8s: { manifests: e.target.value } })}
                />
              </div>
            </div>
          )}
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setService({ ...service, helm: { chart: e.target.value, namespace: service.helm?.namespace || 'default' } })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setService({ ...service, helm: { chart: service.helm?.chart || '', namespace: e.target.value } })
                    }
                  />
                </div>
              </div>
            ...
            <div className="space-y-4 p-4 border rounded-lg bg-orange-50 dark:bg-orange-950">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setService({ ...service, kustomize: { path: e.target.value } })}
                />
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {dependencies.map((dep) => (
                <div key={dep} className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">
            ...
            <Select onValueChange={handleAddDependency}>
              ...
              <SelectContent>
                {existingServices
                  .filter((s) => s.name !== service.name && !dependencies.includes(s.name))
                  .map((s) => (
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={!service.name || (hasRepo && !service.repo?.url) || isCloning}>
            {isCloning ? (
              <>
